...
char Dato()
{
	...
	do
	{
		recibido=UART_cGetChar();
		UART_PutChar(recibido);
		confirma=UART_cGetChar();
	}while(confirma!=5);
	return recibido;
}
...
void Deteccion()
{
	...
	do
	{
		if(Ent1==1)if(PRT0DR & 0x80==0x80)sensor=1;

		else if(Ent2==1)if(PRT0DR & 0x20==0x20)sensor=2;
...
void Disparo()
{
	...
	if(listo_tl1==1 && Cam1==2) TimeLapse(1);
	if(listo_tl2==1 && Cam1==2) TimeLapse(2);
	if(isto_int1==0 && listo_int2==0 && listo_tl2==0 && listo_tl2==0) Reset();
}
